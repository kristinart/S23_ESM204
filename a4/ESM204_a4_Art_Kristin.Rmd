...
## Question 1
First, we estimate and plot a quadratic damage function relating the dollar value of damages to the change in global mean temperature based on the modeled data (Figure 1). We omitted an intercept term since there should be no damages by construction when there is no climate change. 
...
## Question 3
Here, we investigated how discounting influences the calculation of SSC, which is defined as the present discounted value of the stream of future damages caused by one additional ton of CO2. We calculated SCC between 2021-2100 (total of 79 years) for discount rates between 1-8% using the following formula: 
...
## Question 4
Next, we calculate the SCC based on the Ramsey Rule, which the National Academies of Sciences, Engineering, and Medicine advised the government to use in 2017 when discounting within the SCC calculation. The Ramsey Rule is as follows:
$$r = ρ + ηg$$
where:
...
plot_4
```
The discount rate calculated by the Ramsey Rule is `r rr` and the resulting SCC value is $`r rr_SCC`, which is shown in Figure 4.
...
1. Policy A would involve business as usual emissions, without any substantial climate change mitigation. If Policy A is undertaken, there are two possible outcomes. There is a 0.5 probability that warming will occur and follow the baseline trajectory and a 0.5 probability that warming each year will be 1.5 times higher than that in the baseline trajectory. 
$$NPV_A = 0.5* (\sum_{t = 0}^{79}\frac{baseline damage_t}{(1 + r)^t}+  (\sum_{t = 0}^{79}\frac{pulse damage_t}{(1 + r)^t} - cost_{implementation})$$

2. Policy B would involve taking immediate and strong action on climate change. If Policy B is undertaken, there is a probability of 1 that warming will continue until 2050 following the baseline trajectory and then will stabilize at 1.29 degrees and stay that way forever. 
...
#add columns to warming_df
warming_df = warming_df %>% 
  mutate(warming_1.5bl = as.numeric(1.5*warming_baseline),
         est_damage_1.5bl = as.numeric(compute_damages(warming_1.5bl)), #compute damages for 1.5 times baseline
         est_damage_stable_t = as.numeric(compute_damages(warming = 1.29)), #compute damages 
         t2 = as.numeric(t - 29))
...
compute_EPV_B = function(r, c, p){
  ...
  total_pre_2050 = as.numeric(calculate_PV(r = r, damages = warming_df$est_damage_bl, range = 0:29))
  
  #calculate the total cost once temp stabilizes in 2050 
  total_post_2050 = as.numeric(calculate_PV(r = r, damages = warming_df$est_damage_stable_t, range = 0:50)) 
...
table_1
```
We calculated the expected present value of damages up to 2100 under both policies (Table 1). The expected present value (EPV) of damages up to 2100 under Policy A is $`r EPV_A` while the expected present value (EPV) of damages up to 2100 under Policy B is 
$`r EPV_B`. Policy B could cost up to 
$`r EPV_diff` to implement and still make economic sense to pursue over Policy A if Policy A cost 
$0 to implement and the society is risk-neutral. 

If society were risk averse, Policy B would be more attractive because the outcome is certain (due to the 100% probability). In Policy A however, the outcome is uncertain. There is a 50% chance warming will follow the baseline and result in lower damages, but there's also a 50% risk that warming will be 1.5 times higher than the baseline and cause more damages.
...
We began by calculating the net present value of damages up to 2100 under both policies
...
#modify EPV_A function to give output of each potential outcome
#define function to calculate expected present value under policy A
compute_EPV_A2 = function(r, coeff, c, p){
  #calculate the total cost of baseline scenario
  total_bl = as.numeric(calculate_PV(r = r, damages = warming_df$est_damage_bl, range = warming_df$t))
  ...
  #add total benefits, subtract total costs, and multiply by probability 
  EPV_A = (total_bl + total_1.5 - c)*p
  return(list(total_bl, total_1.5, EPV_A))
}

results_A = compute_EPV_A2(r = r, coeff = coeff, c = c, p = p) %>% 
  as.data.frame() %>% 
  rename('total_bl' = 1, 'total_1.5' = 2, 'EPV_A' = 3)
 
total_bl = as.numeric(results_A$total_bl)
total_1.5 = as.numeric(results_A$total_bl)
...
plot_5 = ggplot(df, aes(x = x, y = y))+
  geom_line()+
  geom_point(aes(x = total_bl, y = total_bl/1e15), color = 'coral3', size = 4) +
  geom_point(aes(x = total_1.5, y = total_1.5/1e15), color = 'coral3', size = 4) +
  geom_point(aes(x = EPV_A, y = EPV_A/1e15), color = 'coral3', size = 4) +
  geom_point(aes(x = EPV_B, y = EPV_B/1e15), color = 'coral3', size = 4) +
  #geom_text(aes(x = EPV_A/2, y = EPV_A/2e15), label = "(EPV(A), EU(A))", hjust = -0.2, vjust = 0.2)+
...
plot_6 = ggplot(df, aes(x = x, y = y)) +
  ...
  geom_point(aes(x = total_bl, y = sqrt_fun(total_bl)), color = 'coral3', size = 4) +
  geom_point(aes(x = total_1.5, y = sqrt_fun(total_1.5)), color = 'coral3', size = 4) +
