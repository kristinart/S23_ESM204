...
## Question 3
Here, we investigated how discounting influences the calculation of SSC, which is defined as the present discounted value of the stream of future damages caused by one additional ton of CO2. We calculated and plotted the SSC for discount rates betqween 1-8% and found that higher discount rates result in lower SCC values (Figure 3).
...
plot_4 = plot_3 +
  ...
  geom_text(aes(x = rr, y = rr_SCC), label = "(0.02, 144)", hjust = -0.2, vjust = 0.2)
...
warming_df = warming_df %>% 
  mutate(est_damage_1.5bl = as.numeric(1.5*est_damage_bl), #compute damages for 1.5 times baseline
...
compute_EPV_B = function(r, c, p){
  ...
  total_pre_2050 = as.numeric(calculate_PV(r = r, damages = warming_df$est_damage_bl, range = 0:30 ))#seq(0, 29, 1)))
  
  #calculate the total cost once temp stabilizes in 2050 
  total_post_2050 = as.numeric(calculate_PV(r = r, damages = warming_df$est_damage_stable_t, range = 31:79)) #seq(29, 79, 1)))
...
round(EPV_B2) == round(EPV_A)
(abs((round(EPV_B2))) == abs(round(EPV_A))) #yes, true
```
